...
function ParallaxText({ children, baseVelocity = 100 }: ParallaxProps) {
   ...
   */
  return (
    <div className="overflow-hidden tracking-[-0.3rem] leading-[0.8] m-0 whitespace-nowrap flex flex-nowrap">
      <motion.div
        className="font-extrabold uppercase text-[6rem] flex whitespace-nowrap flex-nowrap"
        style={{ x }}
      >
        <span className="block mr-[0.3rem]">{children} </span>
        <span className="block mr-[0.3rem]">{children} </span>
        <span className="block mr-[0.3rem]">{children} </span>
        <span className="block mr-[0.3rem]">{children} </span>
        <span className="block mr-[0.3rem]">{children} </span>
